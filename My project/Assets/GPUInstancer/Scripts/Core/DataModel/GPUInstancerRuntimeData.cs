...
namespace GPUInstancer
{
    public class GPUInstancerRuntimeData
    {
        ...
        private List<Material> _lodFadeMaterials;
        ...
        public void EnableLODFade(Material mat)
        {
            if (_lodFadeMaterials == null)
                _lodFadeMaterials = new List<Material>();
            if (!_lodFadeMaterials.Contains(mat))
            {
                if (!mat.IsKeywordEnabled(GPUInstancerConstants.KEYWORD_LOD_FADE))
                    mat.EnableKeyword(GPUInstancerConstants.KEYWORD_LOD_FADE);
                _lodFadeMaterials.Add(mat);
            }
        }

        public void DisableLODFade()
        {
            if (_lodFadeMaterials != null)
            {
                foreach (var mat in _lodFadeMaterials)
                {
                    if (mat != null)
                        mat.DisableKeyword(GPUInstancerConstants.KEYWORD_LOD_FADE);
                }
                _lodFadeMaterials = null;
            }
        }
                ...
                for (int r = 0; r < lodRenderers.Count; r++)
                {
                    ...
                    for (int m = 0; m < lodRenderers[r].sharedMaterials.Length; m++)
                    {
                        ...
                        if (prototype.isLODCrossFade && GPUInstancerConstants.gpuiSettings.IsLODCrossFadeSupported())
                            EnableLODFade(instanceMaterials[m]);
                    }
